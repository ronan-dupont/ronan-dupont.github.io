---
layout: archive
title: Curriculum vitæ
permalink: /cv-fr/
lang: fr
translation_key: cv
author_profile: false
profile_page: true
cv_profile:
  kicker: Parcours académique
  headline: Mathématiques appliquées et calcul scientifique
  intro: Je suis chercheur postdoctoral à l’Université de Nagoya. Mes travaux portent sur l’algèbre linéaire numérique
    et les méthodes itératives pour les grands systèmes creux.
  download: Télécharger le CV français
  pdf: /files/Curriculum_Vitae_Ronan_Dupont_FR.pdf?v=20260914
  pdf_label: Édition française
  pdf_note: PDF · 3 pages
  pdf_open: Lire dans le navigateur
  document_title: Le CV complet
  document_description: Formation, expériences de recherche, publications, enseignement et références.
  languages_label: Langues du CV
  career_title: Parcours académique
  career_intro: De la mécanique des fluides numérique et de la modélisation du littoral à l’algèbre linéaire numérique.
  timeline:
  - - Depuis 2026
    - Chercheur postdoctoral
    - Université de Nagoya · Japon
    - Algèbre linéaire numérique, méthodes de Krylov et solveurs creux pour le calcul scientifique.
  - - 2021–2024
    - Doctorat en mathématiques appliquées
    - Université de Montpellier · France
    - Mathématiques appliquées pour la physique numérique, aux laboratoires Géosciences et IMAG. Thèse sur le couplage
      vague–morphodynamique par principe de minimisation.
  - - 2020–2021
    - Master en sciences de la mer
    - Université de Toulon · France
    - Physique de l’environnement pour l’évaluation des risques (PHYMER).
  - - 2018–2021
    - Diplôme d’ingénieur
    - SeaTech, Université de Toulon · France
    - Mécanique des fluides numérique.
  - - 2016–2018
    - Classes préparatoires
    - Caen · France
    - Classes préparatoires aux grandes écoles.
  research_title: Recherche et méthodes
  research:
  - - Algèbre linéaire numérique
    - Méthodes itératives, sous-espaces de Krylov, préconditionnement et systèmes linéaires creux.
  - - Modélisation numérique
    - Éléments virtuels pour la propagation des vagues et approches d’optimisation en morphodynamique littorale.
  - - Programmation scientifique
    - Fortran, Python, MATLAB, C et C++ ; Bash et Slurm pour les calculs scientifiques.
  teaching_title: Enseignement
  teaching_intro: Une expérience dans l’enseignement supérieur et secondaire, en mathématiques, modélisation et
    programmation.
  teaching:
  - - '2025'
    - Mathématiques au lycée à Bordeaux et à l’EFI de Phnom Penh, au Cambodge.
  - - 2023–2024
    - Algèbre, calcul, cardinalité et géométrie en première année de licence à l’Université de Montpellier.
  - - '2023'
    - Cours de master en hydromorphodynamique côtière, modélisation avec OptiMorph et Python.
  teaching_link: Consulter les supports de cours
  teaching_url: /fr/teaching/
  service_title: Conférence et responsabilités scientifiques
  service_date: 14 septembre 2026
  service_event: ICMASE 2026
  service_location: Nagoya, Japon
  service_talk: 'Présentation : GPBiCG(m, ℓ, k) — An Extension of GPBiCG(m, ℓ) and Preliminary Adaptive Parameter Strategies.'
  service_role: 'Responsabilité scientifique : présidence de sessions.'
  service_resources_label: Présentation ICMASE et ressources associées
  service_poster_note: Le poster A0 associé a été préparé pour le projet Moonshot.
  service_links:
  - - Page de la présentation
    - /talks/ICMASE2026/
  - - Diapositives · PDF statique
    - /files/presentation/ICMASE2026_ultra_handout.pdf?v=20
  - - Présentation progressive · PDF
    - /files/presentation/ICMASE2026_ultra_progressive.pdf?v=20
  - - Poster Moonshot associé · PDF A0
    - /files/poster/Moonshot_GPBiCG_MLK_A0_portrait.pdf?v=19
  more_title: Découvrir mes travaux
  more:
  - - Publications
    - Articles et résultats de recherche
    - /fr/publications/
  - - Conférences
    - Présentations et posters
    - /fr/talks/
  - - Contact
    - Échanges scientifiques et professionnels
    - /fr/contact/
---

{% include profile-cv.html %}
