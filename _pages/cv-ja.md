---
layout: archive
title: 履歴書・研究経歴
permalink: /cv-ja/
lang: ja
translation_key: cv
author_profile: false
profile_page: true
cv_profile:
  kicker: 研究者プロフィール
  headline: 応用数学と科学技術計算
  intro: 名古屋大学の博士研究員として、数値線形代数と大規模疎線形方程式に対する反復解法を研究しています。
  download: 日本語CVをダウンロード
  pdf: /files/Curriculum_Vitae_Ronan_Dupont_JP.pdf?v=20260914
  pdf_label: 日本語版
  pdf_note: PDF · 3ページ
  pdf_open: ブラウザで読む
  document_title: CVの全文
  document_description: 学歴、研究経験、研究業績、教育活動、推薦者情報をまとめています。
  languages_label: CVの言語
  career_title: 学歴・研究経歴
  career_intro: 数値流体力学と沿岸域のモデリングから、数値線形代数へ。
  timeline:
  - - 2026年–現在
    - 博士研究員
    - 名古屋大学 · 日本
    - 数値線形代数、Krylov部分空間法、科学技術計算のための疎行列ソルバー。
  - - 2021–2024年
    - 応用数学博士
    - モンペリエ大学 · フランス
    - GéosciencesおよびIMAG研究所で計算物理のための応用数学を研究。博士論文では、最小化原理に基づく波浪・地形変化の結合モデルを扱いました。
  - - 2020–2021年
    - 海洋科学修士
    - トゥーロン大学 · フランス
    - リスク評価のための環境物理（PHYMER）。
  - - 2018–2021年
    - 工学ディプロマ
    - SeaTech・トゥーロン大学 · フランス
    - 数値流体力学。
  - - 2016–2018年
    - グランゼコール準備課程
    - カーン · フランス
    - フランスの工学系グランゼコールへの進学準備課程。
  research_title: 研究分野と手法
  research:
  - - 数値線形代数
    - 反復法、Krylov部分空間法、前処理、大規模疎線形方程式。
  - - 数値モデリング
    - 波浪伝播に対する仮想要素法と、沿岸地形変化を扱う最適化手法。
  - - 科学技術プログラミング
    - Fortran、Python、MATLAB、C、C++。科学技術計算のためのBashとSlurm。
  teaching_title: 教育経験
  teaching_intro: 大学と高校で、数学、モデリング、プログラミングの教育に携わってきました。
  teaching:
  - - 2025年
    - フランス・ボルドーおよびカンボジア・プノンペンのEFIで高校数学を担当。
  - - 2023–2024年
    - モンペリエ大学の学部1年次に、代数、微積分、濃度、幾何を担当。
  - - 2023年
    - 修士課程で沿岸域の水理・地形変化、OptiMorphによるモデリング、Pythonを担当。
  teaching_link: 教材を見る
  teaching_url: /ja/teaching/
  service_title: 学会発表・学術活動
  service_date: 2026年9月14日
  service_event: ICMASE 2026
  service_location: 日本・名古屋
  service_talk: '発表：GPBiCG(m, ℓ, k) — An Extension of GPBiCG(m, ℓ) and Preliminary Adaptive Parameter Strategies.'
  service_role: 学術活動：セッション座長。
  service_resources_label: ICMASE発表と関連資料
  service_poster_note: 関連するA0ポスターはMoonshotプロジェクトのために作成したものです。
  service_links:
  - - 発表紹介
    - /talks/ICMASE2026/
  - - 配布用スライド · PDF
    - /files/presentation/ICMASE2026_ultra_handout.pdf?v=20
  - - 段階表示スライド · PDF
    - /files/presentation/ICMASE2026_ultra_progressive.pdf?v=20
  - - 関連Moonshotポスター · A0 PDF
    - /files/poster/Moonshot_GPBiCG_MLK_A0_portrait.pdf?v=19
  more_title: 研究活動を見る
  more:
  - - 研究業績
    - 論文と研究成果
    - /ja/publications/
  - - 講演
    - 発表資料とポスター
    - /ja/talks/
  - - お問い合わせ
    - 研究・仕事に関する連絡
    - /ja/contact/
---

{% include profile-cv.html %}
