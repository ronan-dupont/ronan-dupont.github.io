...
from optimorph.utils_functions import (k_guo, calc_H, shoreline, shoaling_window_RONAN, slopecst, 
                                        ...
                                        trapz_2D,func_to_plot_2D,func_to_plot_2D_flat,H_shoaling_2D,
                                        Dif_Hadamard_2D,lim_pente_1,limiteur_2D,vec_n,
                                        calc_ponderatau, calc_dumping, dif_centre_neuman, calc_vect_t,
                                        apply_max_displacement, cut_boundary_minidepth, apply_sand_conservation,
                                        apply_post_sand_conservation, calc_dumping_inv, dif_LaxWendroff)
...
def main(output_dir: Union[str, Path], config_file: Union[str, Path], psi_datas: Union[str, Path]):
    ...
    if not(two_dimension):
        ...
        for itera in range(niter):
            ...
            ponderatau = calc_ponderatau(ntot, H_t, h_t, h_pondera, minidepth)
            
            dumping = calc_dumping(ntot, x, k_t, h_t, minidepth)
            dumping_inv = calc_dumping_inv(ntot, x, k_t, h_t, minidepth)
            ...
            dj*=-rho0*dumping*ponderatau   # dj only vertocal 

            # Transport
            save_psi = psi.copy()
            mobility2 = 1e-2
            mobility2
            psi_lin = savgol_filter(psi, 50, 1)
            
            psi -= psi_lin
            
            V = dumping_inv**3 * dumping * H_t * mobility2 #* u 
            
            alpha = 0.9
            dt =  alpha * dx #/ max(V) 
            alpha = dt / dx
            psi_x = dif_LaxWendroff(alpha, psi)

            psi = psi - psi_x * V
            
            #print(round(max(dj),7),round(max(psi_x * V),7))
            psi[0] = psi[1]
            psi[ntot-1] = psi[ntot-2]
            
            psi += psi_lin # puting repere back
            
            # End Transport
            
            dj = apply_max_displacement(dj, psi, psi0, ntot, depl_max, depl_total_max_absolu, max_loss_depth)
                   
            psi1= slopecst(psi + dj, Mslope, rho0, x)  # pente max implicite avt project
            dj = psi1-psi
            
            dj = cut_boundary_minidepth(dj,h_t,minidepth,ntot)
            
            dj = apply_sand_conservation(dj, psi, psi0, x, h_t, minidepth)

            d = cut_boundary_minidepth(dj,h_t,minidepth,ntot)
            
            psi += d
                
            psi = apply_post_sand_conservation(psi, sand0, x, h_t, minidepth)      #contrainte conservation a posteriori
