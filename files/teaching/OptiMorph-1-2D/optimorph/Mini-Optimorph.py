...
def main(output_dir: Union[str, Path], config_file: Union[str, Path], psi_datas: Union[str, Path]):
    ...
    if not(two_dimension):
        ...
        for itera in range(niter):
            ...
            # Transport part
            """
            psi_x = dif_centre_neuman(psi, xstep)
            psi_xx = dif_centre_neuman(psi_x, xstep)
            t_x, t_y = calc_vect_t(psi, xstep)
            
            eps_x = 1e-2
            #mobility2 = 100
            
            # What the best way to define V ? 
            V = (dj * abs(t_y)) / (abs(psi_x) + eps_x * dx ) #* (1-dumping)
            
            V = lim_pente_1(V)
            viscosity = 0.5 * dx * abs(V) * psi_xx # 0.5 h u psi_xx
            
            #dj += - mobility2 * V * psi_x + viscosity
            """ 
